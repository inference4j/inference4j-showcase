...
async function detect() {
  ...
  try {
    ...
    annotatedImg.src = `data:image/png;base64,${data.image}`;
...
function renderTable(detections) {
  ...
  if (detections.length === 0) {
    detectionsTable.innerHTML = '<p class="text-muted" style="padding:0.5rem 0">No objects detected.</p>';
    return;
  }

  detections
    .sort((a, b) => b.confidence - a.confidence)
    .forEach(({ label, confidence, color }) => {
      const pct = (confidence * 100).toFixed(1);
      const row = document.createElement('div');
      row.className = 'detection-row';
      row.innerHTML = `
        <span class="detection-swatch" style="background:${color}"></span>
